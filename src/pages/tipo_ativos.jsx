...
export default function Home() {
    ...
    async function getLista() {
        const res = await axios.get('/api/tipo_ativo');
        setLista(res.data);
    }

    async function salvar() {
        if (_id === '') {
            await axios.post('/api/tipo_ativo', {
                nome
            });
        } else {
            await axios.put('/api/tipo_ativo', {
                _id,
                nome
            });
        }
        toast.success('Tipo de ativo cadastrado com sucesso.');
        setNome('');
        setId('');
        getLista();
    }
    ...
    async function updateStatus(_id, status) {
        ...
        toast.success(`Tipo de ativo ${!status? 'bloqueado': 'ativado'} com sucesso.`)
        getLista();
    }

    async function deleteItem(_id){
        await axios.delete('/api/tipo_ativo', {
            data: {
                _id
            }
        })
        .then(()=>{
            toast.success("Tipo de ativo deletado com sucesso.");
            getLista();
        })
    }
